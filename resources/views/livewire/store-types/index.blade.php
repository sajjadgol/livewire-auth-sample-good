    ...
    <div class="row mt-4">
        <div class="col-12">
            ...
            <div class="card custom-card">
                <!-- Card header -->
                @include('livewire.store-types.filter')
                <!-- Card header end -->
                        ...
                        <x-table.row wire:key="row-{{ $storeType->id }}">
                            ...
                            <x-table.cell> 
                                <div class="form-check form-switch ms-3">
                                    <input class="form-check-input" wire:loading.attr="disabled"  type="checkbox" id="flexSwitchCheckDefault35"  wire:change="statusUpdate({{ $storeType->id }},{{ $storeType->status}})"
                            ...
                            <x-table.cell> 
                                ...
                                <a href="@if(app()->getLocale() != $locale) {{ route('edit-store-type', ['id' => $storeType->id,'ref_lang' => $locale]) }}  @else {{ route('edit-store-type', $storeType) }} @endif" class="" data-original-title="{{ $locale }}" title="{{ $locale }}"> 
                                    <span class="material-symbols-outlined text-md">
                                     {{ in_array($locale, array_column(json_decode($storeType->translations, true), 'locale')) ? 'edit' : 'add' }}
                                   </span>
                                </a> 
                                @endforeach
                            </x-table.cell>
                ...
                @if($storeTypes && $storeTypes->total() > 10)
                    <div class="row mx-2">
