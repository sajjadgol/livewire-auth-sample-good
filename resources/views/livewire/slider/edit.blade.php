        ...
        <x-slot name="body">
            ...
            <x-form.form submit-target="store" cancel-route="{{ route('store-type-management') }}">
                ...
                <x-input.group colspan="col-12" for="name" label="Name *" :error="$errors->first('slider.name')">
                    {{-- Input text --}}
                    <x-input.text wire:model.lazy="slider.name" placeholder="Enter a Name" />
                </x-input.group>

                <x-input.group colspan="col-12" for="description" label="Description *" :error="$errors->first('slider.description')">
                    {{-- Rech text --}}
                    <x-input.rich-text wire:model.lazy="slider.name">
                        {{ $slider->description }}
                    </x-input.rich-text>
                </x-input.group>
...
<x-modal wire:ignore.self  id="exampleModal" >
    <x-slot name="title">
    ...
    <x-slot name="footer">
        ...
        <x-button.bg-gradient-dark  wire:click.prevent='storeImage()' id="storeImage">Save</x-button.bg-gradient-dark>
    </x-slot>
</x-modal>
