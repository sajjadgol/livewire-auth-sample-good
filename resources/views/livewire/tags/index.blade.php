                    ...
                    <x-slot name="body">
                        ...
                        <x-table.row wire:key="row-{{ $tag->id }}">
                            ...
                            <x-table.cell> <div class="form-check form-switch ms-3">
                                <input class="form-check-input" wire:loading.attr="disabled"  type="checkbox" id="flexSwitchCheckDefault35"  wire:change="statusUpdate({{  $tag->id }},{{ $tag->status}})"
                                    @if( $tag->status) checked="" @endif>
                            </div></x-table.cell>
                            <x-table.cell> 
                                @foreach (config('translatable.locales') as $locale)
                                <a href="@if(app()->getLocale() != $locale) {{ route('edit-product-tag', ['id' => $tag->id,'ref_lang' => $locale]) }}  @else {{ route('edit-product-tag', $tag) }} @endif" class="" data-original-title="{{ $locale }}" title="{{ $locale }}"> 
                                    <span class="material-symbols-outlined text-md">
                                     {{ in_array($locale, array_column(json_decode($tag->translations, true), 'locale')) ? 'edit' : 'add' }}
                                   </span>
                                </a> 
                                @endforeach
                            </x-table.cell>
