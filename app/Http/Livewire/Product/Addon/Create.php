...
class Create extends Component
{
    ...
    protected $rules = [
        'name' => 'required|max:255|unique:App\Models\Product\AddonOptionTranslation,name',
    ...
    {    
    $this->optionValues->push(['product_addon_option_id '=> '','option' => '','price' => 0]);
    }
...
public function mount()
    {
    $this->fill([
        'optionValues' => collect([['product_addon_option_id '=> '', 'option' => '','price' => 0]]),
    ]);
    }
    ...
    public function store(){
        ...
        foreach($this->optionValues as $key => $value){
            AddonOptionValue::create(['product_addon_option_id'=> $addons->id, 'name' => $value['option'], 'price' => $value['price'], 'created_at' => Carbon::now(), 'updated_at'=> Carbon::now()]);
        }
    
        return redirect(route('product-addon-management'))->with('status',__('product.AddonOption successfully created'));
    }
