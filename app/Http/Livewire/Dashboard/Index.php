    ...
    public function mount() {
        ...
        if(auth()->user()->hasRole('Provider')){
            ...
            $orderRevenues = [];                           
                ...
                case 'pgsql':
                    $orderRevenues_query = order::whereDate("created_at",">=",  Carbon::now()->subMonths(6)->firstOfMonth())
                    ->select(DB::raw('count(id)
 as total'),DB::raw("DATE_PART('year', created_at::date) as year"),DB::raw("DATE_PART('month',created_at::date) as months"),DB::raw("TO_CHAR(created_at::date,'Mon') as month"),DB::raw('COUNT("order_status") filter (where order_status = '."'completed'".') AS completed'))
                    ->whereIn('order_status',['completed', 'pending', 'cancelled', 'refund'])
                    ->groupby('year','months','month');
                break;
                case 'mysql':
                    $orderRevenues_query = order::whereDate("created_at",">=",  Carbon::now()->subMonths(6)->firstOfMonth())
                    ->whereIn('order_status',['completed', 'pending', 'cancelled', 'refund'])
                    ->select(DB::raw('count(id)  as total'), DB::raw('DATE_FORMAT(created_at, "%b") as month'), DB::raw('COUNT((SELECT order_status="completed" where order_status="completed")) as completed'),DB::raw('YEAR(created_at) year, MONTH(created_at) months'))->groupby('year','months');
                break;
                ...
                if(auth()->user()->hasRole('Provider')){	
                    $orderRevenues_query->where('store_id', $this->getStoreId());
                }
                $orderRevenues = $orderRevenues_query->get();
